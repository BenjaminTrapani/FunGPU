...
public:
  ...
  static constexpr Index_t MAX_BLOCKS_SCHEDULED_PER_PASS =
      std::min(MaxNumReactivations / RuntimeBlockType::NumThreadsPerBlock,
               MAX_NUM_BLOCKS_ALLOCATED_PER_PASS);
  ...
  struct Buffers {
    ...
    std::vector<std::deque<IndirectCallRequest>> buffered_indirect_call_requests;
  ...
  static void setup_block_for_lambda(
      ...
      Program,
      cl::sycl::accessor<Index_t, 1, cl::sycl::access::mode::read_write,
                         cl::sycl::access::target::global_buffer> total_indirect_calls_across_scheduled_blocks_acc);
...
void IndirectCallHandler<RuntimeBlockType, MaxNumIndirectCalls,
  ...
  const cl::sycl::atomic_ref<Index_t, cl::sycl::memory_order::seq_cst,
                             cl::sycl::memory_scope::device,
                             cl::sycl::access::address_space::global_space>
      total_indirect_calls_across_scheduled_blocks(total_indirect_calls_across_scheduled_blocks_acc[0]);
  for (Index_t i = 0; i < num_blocks_required; ++i) {
    ...
    if (const auto total_calls = total_indirect_calls_across_scheduled_blocks.fetch_add(instructions_data.instruction_properties.total_num_indirect_calls);
        total_calls + instructions_data.instruction_properties.total_num_indirect_calls > MaxNumIndirectCalls) {
      total_indirect_calls_across_scheduled_blocks.fetch_sub(instructions_data.instruction_properties.total_num_indirect_calls);
      break;
    }
...
void IndirectCallHandler<RuntimeBlockType, MaxNumIndirectCalls,
  ...
  for (Index_t i = 0;
       ...
       ++i) {
    const auto total_num_indirect_calls = mem_pool_acc[0].deref_handle(block_reactivation_requests_by_block[0].runtime_blocks_reactivated[i].lambda)->instruction_properties.total_num_indirect_calls;
    if (const auto total_calls = total_indirect_calls_across_scheduled_blocks.fetch_add(total_num_indirect_calls);
        total_calls + total_num_indirect_calls > MaxNumIndirectCalls) {
      total_indirect_calls_across_scheduled_blocks.fetch_sub(total_num_indirect_calls);
      break;
    }
    ...
    const auto& lambda = *mem_pool_acc[0].deref_handle(block_reactivation_requests_by_block[0].runtime_blocks_reactivated[i].lambda);
...
template <typename RuntimeBlockType, Index_t MaxNumIndirectCalls,
          Index_t MaxNumReactivations>
void IndirectCallHandler<RuntimeBlockType, MaxNumIndirectCalls,
                         MaxNumReactivations>::BlockReactivationRequestBuffer::append(const typename RuntimeBlockType::BlockMetadata & block_metadata) {
     cl::sycl::atomic_ref<Index_t, cl::sycl::memory_order::seq_cst,
                       cl::sycl::memory_scope::device,
                       cl::sycl::access::address_space::global_space>
...
auto IndirectCallHandler<RuntimeBlockType, MaxNumIndirectCalls,
  ...
  // If there are any host-side buffered calls and space exists in indirect call buffer, append queued calls.
  {
    std::optional<cl::sycl::accessor<IndirectCallRequestBuffer, 1,
                           cl::sycl::access::mode::read_write,
                           cl::sycl::access::target::host_buffer>> indirect_call_acc;
    for (Index_t lambda_idx = 0; lambda_idx < program.get_count(); ++lambda_idx) {
        auto &buffered_indirect_call_requests = buffers.buffered_indirect_call_requests[lambda_idx];
        if (buffered_indirect_call_requests.empty()) {
            continue;
        }
        if (!indirect_call_acc.has_value()) {
            indirect_call_acc = buffers.indirect_call_requests_by_block.template get_access<cl::sycl::access::mode::read_write>();
        }
        auto &indirect_call_reqs = (*indirect_call_acc)[lambda_idx];
        while (!buffered_indirect_call_requests.empty() && indirect_call_reqs.num_indirect_call_reqs < MaxNumIndirectCalls) {
            indirect_call_reqs.append(buffered_indirect_call_requests.front());
            buffered_indirect_call_requests.pop_front();
        }
    }
  }
  // If there are any host-side buffered reactivations and space exists in reactivation buffer, append queued reactivations.
  if (!buffers.buffered_reactivations.empty()){
    auto block_reactivation_requests_by_block_acc = buffers.block_reactivation_requests_by_block.template get_access<cl::sycl::access::mode::read_write>();
    while (!buffers.buffered_reactivations.empty() && block_reactivation_requests_by_block_acc[0].num_reactivations_scheduled < MaxNumReactivations) {
      block_reactivation_requests_by_block_acc[0].append(buffers.buffered_reactivations.front());
  ...
  work_queue.submit([&](cl::sycl::handler &cgh) {
    ...
    cgh.parallel_for<class BlockGroupsPerLambda>(
        ...
        [mem_pool_write, block_exec_group_acc, program,
         ...
         num_blocks_scheduled_acc, total_indirect_calls_across_scheduled_blocks_acc](cl::sycl::item<1> itm) {
          ...
          if (lambda_idx < program.get_count()) {
            ...
            setup_block_for_reactivation(
                ...
                num_blocks_scheduled_acc,
                max_num_instructions_acc, total_indirect_calls_across_scheduled_blocks_acc);
          }
        });
  });
  ...
  // Move unscheduled calls to a separate buffer. TODO optimize to avoid device to host transfer in common case.
  {
    auto indirect_call_requests_by_block_acc =
          buffers.indirect_call_requests_by_block
              .template get_access<cl::sycl::access::mode::read>();
    for (Index_t lambda_idx = 0; lambda_idx < program.get_count(); ++lambda_idx) {
      const auto &indirect_call_reqs = indirect_call_requests_by_block_acc[lambda_idx];
      for (Index_t thread_idx = indirect_call_reqs.num_indirect_calls_allocated; thread_idx < indirect_call_reqs.num_indirect_call_reqs; ++thread_idx) {
        const auto &req = indirect_call_reqs.indirect_call_requests[thread_idx];
        std::cout << "Adding buffered indirect call request for lambda " << lambda_idx << " thread " << thread_idx << 
            ", indirect_call_reqs.num_indirect_calls_allocated=" << indirect_call_reqs.num_indirect_calls_allocated << 
            ", indirect_call_reqs.num_indirect_call_reqs=" << indirect_call_reqs.num_indirect_call_reqs << std::endl;
...

  {
    ...
    for (Index_t pending_reactivation_idx = reactivation_buffer_acc[0].num_reactivations_scheduled; pending_reactivation_idx < reactivation_buffer_acc[0].num_runtime_blocks_reactivated; ++pending_reactivation_idx) {
      std::cout << "Adding buffered reactivation" << std::endl;
      buffers.buffered_reactivations.push_back(reactivation_buffer_acc[0].runtime_blocks_reactivated[pending_reactivation_idx]);
    }
  }
  ...
  });
  {
    ...
    auto max_num_instructions_acc =
        buffers.max_num_instructions
            .template get_access<cl::sycl::access::mode::read>();
    return BlockExecGroup{.num_blocks = std::min(num_blocks_scheduled_acc[0], MAX_BLOCKS_SCHEDULED_PER_PASS),
