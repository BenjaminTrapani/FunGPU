...
Compiler::compile_list_of_sexpr(std::shared_ptr<const SExpr> sexpr,
  ...
  if (first_child->get_type() == SExpr::Type::Symbol) {
    ...
    if (first_child_sym == "+") {
      ...
      result = mem_pool_acc[0].template alloc<BinaryOpNode>(
          ASTNode::Type::Add,
          compile(sexpr_children.at(1), bound_identifiers, all_identifiers,
                  mem_pool_acc),
          compile(sexpr_children.at(2), bound_identifiers, all_identifiers,
                  mem_pool_acc));
      ...
      for (const auto &bind_expr : binding_exprs) {
        ...
        all_identifiers.push_back(ident_expr->get_symbol());
      }
      ...
      for (Index_t i = 0; i < binding_exprs.size(); ++i) {
        ...
        const auto &bind_expr_children = bind_expr->get_children();
        bindings_data[i] =
            compile(bind_expr_children.at(1),
                    is_rec ? updated_bindings : bound_identifiers,
                    all_identifiers, mem_pool_acc);
      }

      bind_node->m_child_expr = compile(sexpr_children.at(2), updated_bindings,
                                        all_identifiers, mem_pool_acc);
      ...
      for (const auto &identifier : identifier_list_children) {
        ...
        all_identifiers.push_back(identifier->get_symbol());
      }
      auto expr_to_eval = sexpr_children.at(2);
      auto compiled_ast_node = compile(expr_to_eval, bound_identifiers,
                                       all_identifiers, mem_pool_acc);
      ...
      result = mem_pool_acc[0].template alloc<IfNode>(
          compile(pred_child, bound_identifiers, all_identifiers, mem_pool_acc),
          compile(then_child, bound_identifiers, all_identifiers, mem_pool_acc),
          compile(else_child, bound_identifiers, all_identifiers,
                  mem_pool_acc));
      ...
      result = mem_pool_acc[0].template alloc<UnaryOpNode>(
          ASTNode::Type::Floor, compile(sexpr_children.at(1), bound_identifiers,
                                        all_identifiers, mem_pool_acc));
    }
  }
  if (result ==
      ASTNodeHandle()) // This is hopefully a call to user-defined function.
  {
    ...
    const auto call_node_handle = mem_pool_acc[0].template alloc<CallNode>(
        ...
        compile(target_lambda_expr, bound_identifiers, all_identifiers,
                mem_pool_acc),
        mem_pool_acc);
    ...
    for (Index_t i = 1; i < sexpr_children.size(); ++i) {
      ...
      args_data[i - 1] =
          compile(cur_arg, bound_identifiers, all_identifiers, mem_pool_acc);
    }
...
ASTNodeHandle Compiler::compile(std::shared_ptr<const SExpr> sexpr,
  ...
  case SExpr::Type::ListOfSExpr: {
    result = compile_list_of_sexpr(sexpr, bound_identifiers, all_identifiers,
                                   mem_pool_acc);
    break;
  }
  default:
