...
void BlockGenerator::assign_lambdas_block_indices(
    ...
    std::map<Compiler::ASTNodeHandle, Index_t> &result,
    Index_t &cur_index) {
  visit(*mem_pool_acc[0].derefHandle(root), Visitor {
   [&](const Compiler::LambdaNode &bind_node) {
      if (!result.emplace(root, cur_index++).second) {
        throw std::invalid_argument(
            "Multiple paths to the same lambda node in tree");
      }
      assign_lambdas_block_indices(
          mem_pool_acc, bind_node.m_childExpr,
          result, cur_index);
    },
    [&](const auto &node) {
      node.for_each_sub_expr(mem_pool_acc, [&](const auto &sub_expr) {
        assign_lambdas_block_indices(mem_pool_acc, sub_expr, result,
                                        cur_index);
      });
    }
  }, [](const auto&) {
...
void BlockGenerator::compute_lambda_space_ident_to_use_count(
    const Compiler::ASTNodeHandle &node,
    Index_t num_bound_so_far,
    PortableMemPool::HostAccessor_t &mem_pool_acc,
    std::unordered_map<Index_t, Index_t>
        &lambda_space_ident_to_remaining_count) {
  visit(*mem_pool_acc[0].derefHandle(node), Visitor {
    [&](const Compiler::BindNode& bind_node) {
      ...
      compute_lambda_space_ident_to_use_count(bind_node.m_childExpr, num_bound_so_far, mem_pool_acc, lambda_space_ident_to_remaining_count);
    },
    [&](const Compiler::LambdaNode& lambda_node) {
      ...
      lambda_node.for_each_sub_expr(mem_pool_acc, [&](const auto& elem) {
        compute_lambda_space_ident_to_use_count(elem, num_bound_so_far, mem_pool_acc, lambda_space_ident_to_remaining_count);
      });
    },
...
void BlockGenerator::extract_lambda_space_captured_indices(
  ...
  const auto recurse_on_subexprs = [&](const auto& sub_expr) {
    sub_expr.for_each_sub_expr(mem_pool_acc, [&](const auto& elem) {
      extract_lambda_space_captured_indices(elem, num_bound_so_far, mem_pool_acc, captured_indices);
    });
  };
  ...
  visit(*mem_pool_acc[0].derefHandle(node), Visitor {
    ...
    [&](const Compiler::IdentifierNode& ident_node) {
      if (ident_node.m_index >= num_bound_so_far) {
        captured_indices.emplace(num_bound_so_far - ident_node.m_index);
      }
    },
    [&](const auto& sub_node) {
      recurse_on_subexprs(sub_node);
    }
  }, [](const auto&) {
    throw std::invalid_argument("Unexpected node");
  });
...
Lambda BlockGenerator::construct_block(
    ...
    const std::map<Compiler::ASTNodeHandle, Index_t>
        &lambdas_to_blocks,
    PortableMemPool::HostAccessor_t &mem_pool_acc) {
  ...
  {
    std::set<Index_t> captured_indices;
    extract_lambda_space_captured_indices(lambda, 0, mem_pool_acc, captured_indices);
  ...
  const auto primitive_expression_to_instruction = [&](const Compiler::ASTNodeHandle ast_node) {
    ...
    visit(*mem_pool_acc[0].derefHandle(ast_node), Visitor {
       [&](const Compiler::CallNode &call_node) {
        ...
        const auto *args_data = mem_pool_acc[0].derefHandle(call_node.m_args);
        const auto arg_indices =
            mem_pool_acc[0].AllocArray<Index_t>(call_node.m_args.GetCount());
        auto *arg_indices_data = mem_pool_acc[0].derefHandle(arg_indices);
        for (Index_t arg_idx = 0; arg_idx < arg_indices.GetCount(); ++arg_idx) {
          const auto &arg_elem =
      ...
      [&](const Compiler::LambdaNode &) {
        ...
        result.data.create_lambda.block_idx = lambda_to_block_idx_iter->second;
        const auto captured_indices = [&] {
          std::set<Index_t> captured_indices_set;
          extract_lambda_space_captured_indices(ast_node, 0, mem_pool_acc, captured_indices_set);
          return std::vector<Index_t>(captured_indices_set.begin(), captured_indices_set.end());
        }();
        ...
        for (Index_t i = 0; i < captured_indices_handle.GetCount(); ++i) {
          ...
          if (register_idx_iter == lambda_space_ident_to_register.end()) {
            throw std::invalid_argument(
                "Failed to find register for captured value in nested lambda");
          }
          captured_indices_data[i] = register_idx_iter->second;
        }
        result.data.create_lambda.captured_indices = captured_indices_handle;
        ...
        result.type = [&] () -> InstructionType {
            switch(binary_op_node.m_type) {
              ...
              case Compiler::ASTNode::Type::Lambda:
                throw std::invalid_argument("Unexpected AST node type for binary op");
            }
        }();
        visit(result, Visitor{
          [&]<InstructionType TYPE>(BinaryOp<TYPE>& instruction) {
            instruction.lhs = lookup_register_for_ident(*mem_pool_acc[0].derefHandle(binary_op_node.m_arg0));
      ...
      [&](const Compiler::UnaryOpNode& unary_op_node) {
        ...
        visit(result, Visitor{
          [&](Floor& floor) {
            floor.arg = lookup_register_for_ident(*mem_pool_acc[0].derefHandle(unary_op_node.m_arg0));
      ...
      [&](const Compiler::BindNode&) {
        throw std::invalid_argument("Should not generate instructions for bind nodes");
      },
      [&](const Compiler::IdentifierNode&) {
        throw std::invalid_argument("Duplicate identifiers should be removed during block prep");
      }
    }, [](const auto &) {
    ...
    CollectAllASTNodes(ast_node, mem_pool_acc, {Compiler::ASTNode::Type::Identifier}, identifiers);
    for (const auto ident : identifiers) {
      const auto& ident_node = static_cast<const Compiler::IdentifierNode&>(*mem_pool_acc[0].derefHandle(*ident));
      const auto ident_in_lambda_space =
          convert_to_lambda_space_index(ident_node.m_index);
      auto iter =
...
Program
BlockGenerator::construct_blocks(const Compiler::ASTNodeHandle root) {
  auto mem_pool_acc = pool_.template get_access<cl::sycl::access::mode::read_write>();
  Index_t initial_index = 0;
  std::map<Compiler::ASTNodeHandle, Index_t> lambdas_to_blocks;
  assign_lambdas_block_indices(
      mem_pool_acc, root, lambdas_to_blocks, initial_index);
  const auto all_lambdas_handle = mem_pool_acc[0].AllocArray<Lambda>(lambdas_to_blocks.size());
