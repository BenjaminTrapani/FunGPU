...
struct Fixture {
  ...
  void check_block_evaluates_to_value(const float_t val, const RuntimeBlockType::BlockMetadata block_meta) {
    work_queue.submit([&](cl::sycl::handler& cgh) {
       auto mem_pool_write =
          mem_pool_buffer.get_access<cl::sycl::access::mode::read_write>(cgh);
       cl::sycl::accessor<RuntimeBlockType, 1, cl::sycl::access::mode::read_write, cl::sycl::access::target::local> local_block(
       ...
       auto results_acc = results.get_access<cl::sycl::access::mode::discard_write>(cgh);
       cgh.parallel_for<class TestEvalLoop>(
         cl::sycl::nd_range<1>(THREADS_PER_BLOCK, THREADS_PER_BLOCK),
         [mem_pool_write, block_meta, local_block, local_instructions, results_acc](cl::sycl::nd_item<1> itm) {
           const auto thread_idx = itm.get_global_linear_id();
           if (thread_idx == 0) {
             local_block[0] = *mem_pool_write[0].derefHandle(block_meta.block);
           }
           const auto* instructions_global_data = mem_pool_write[0].derefHandle(block_meta.instructions);
           for (auto idx = thread_idx; idx < block_meta.instructions.GetCount(); idx += THREADS_PER_BLOCK) {
    ...
    {
      const auto result_acc = results.get_access<cl::sycl::access::mode::read>();
  ...
  void check_first_lambda_evaluates_to(const float_t expected_val, const std::string& program) {
    ...
    const auto block_meta = [&] {
      ...
      return RuntimeBlockType::BlockMetadata(block_handle, lambdas[0].instructions);
    }();
...
BOOST_FIXTURE_TEST_CASE(EvaluateFalseBranchInTailPos, Fixture) {
  check_first_lambda_evaluates_to(5, "./TestPrograms/FalseBranchInTailPos.fgpu");
}
