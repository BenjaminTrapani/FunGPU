...
RuntimeBlockHandle Evaluator::DependencyAggregator::runtime_block(const Index_t index) {
  return buffers[cur_buffer_idx_][index];
}

void Evaluator::DependencyAggregator::add_block(const RuntimeBlockHandle handle) {
...
auto Evaluator::construct_initial_block(const Program& program) -> RuntimeBlockHandle {
...
RuntimeValue Evaluator::read_result(const RuntimeBlockHandle first_block, const Program& program) {
  auto mem_pool_acc = mem_pool_buffer_.get_access<cl::sycl::access::mode::read_write>();
  const auto& main_lambda = mem_pool_acc[0].derefHandle(program)[0];
