...
namespace FunGPU {
  ...
  public:
    ...
    Compiler::ASTNodeHandle PrepareForBlockGeneration(Compiler::ASTNodeHandle root);
  ...
  private:
    /*
     ...
     (lower layers removed) into an indirect call. Arguments are all the dependencies of
     the instructions in the lower layers removed. All remaining layers of instructions should be
     moved into the construction of the new synthetic lambda. Synthetic lambda
     should be created at the top level of the current closure. Add lambda to
     set of pending programs to generate.
     ...
     for i in let_blocks:
       for j in let_blocks[i]:
            instructions[i][j] = build_instruction_from_primitive_op(let_blocks[i][j])
     tail_op = build_instruction_from_primitive_op(let_blocks[len(let_blocks) - 1].bound_expr)
     ...
     expression and is equal to the total number of nested let blocks.
       ...
       When lambda is evaluated (lambda instruction encountered), dynamically allocate array of runtime values for closed values. Copy the enclosed values (specified in lambda
     instruction) into the array. Attach array handle to lambda val in addition
     to the lambda idx. Store in target register for instruction.
